...
const dummyData = JSON.parse(
    `{ "title": "코딩의 고난과 역경", 
    "thumbnail": "https://api.unsplash.com/search/photos?query=temptation", "summary": "코딩 강의를 듣고 프로젝트 버그로 고민하다가 gpt로 해결함.", 
...
function App() {
    ...
    const handleClickAPIUnsplash = async () => {
        ...
        searchImages(dummyData.thumbnail);
    };

    const searchImages = async (thumbnail) => {
        // "https://api.unsplash.com/search/photos?query=temptation"
        const query = thumbnail.split("=")[1];
        console.log("searchImages", query);
        try {
            const image = await Unsplash({ query });
            console.log(image.results);
            setImages(image.results);
        } catch (error) {
            messageApi.open({
                type: "error",
                content: `이미지 오류 :${error?.message}`,
            });
        }
    };
    return (
        <AppContainer>
            ...
            />
            <DiaryDisplay isLoading={isLoading} data={data} images={images} />
        </AppContainer>
    );
}
